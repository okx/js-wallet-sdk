...
describe("bitcoin", () => {
    ...
    test("message sign", async () => {
        const wif = private2Wif(base.fromHex("adce25dc25ef89f06a722abdc4b601d706c9efc6bc84075355e6b96ca3871621"))
        const s = bitcoin.message.sign(wif, "hello world")
        console.log(s);

        const publicKey = bitcoin.wif2Public(wif);
        const address = bitcoin.payments.p2wpkh({ pubkey: publicKey, network: networks.bitcoin }).address;
        const s2 = await bitcoin.bip0322.signSimple("hello world", address!, wif)
        console.log(s2);

        const taprootAddress = bitcoin.payments.p2tr({ internalPubkey: publicKey.slice(1), network: networks.bitcoin }).address;
        const s3 = await bitcoin.bip0322.signSimple("hello world", taprootAddress!, wif)
        console.log(s3);
    });
});
