...
export class TransactionBlock {
  ...
  // The current default i
  /**
   * Prepare the transaction by valdiating the transaction data and resolving all inputs
   * so that it can be built into bytes.
   */
  async #prepare({  onlyTransactionKind }: BuildOptions) {
    ...
    if (moveModulesToResolve.length) {
      throw new Error(
          `No provider passed to Transaction#build, but transaction data has move modules to resolve.`,
      );
    }

    if (objectsToResolve.length) {
      throw new Error(
          `No provider passed to Transaction#build, but transaction data has objects to resolve.`,
      );
    }
