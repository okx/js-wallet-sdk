...
const stark = signUtil.schnorr.stark;
...
export function keccakBn(value: BigNumberish): string {
  ...
  return addHexPrefix(stark.keccak(hexToBytes(addHexPrefix(evenHex))).toString(16));
}

function keccakHex(value: string): string {
  return addHexPrefix(stark.keccak(utf8ToArray(value)).toString(16));
}
...
export function computeCompiledClassHash(casm: CompiledSierraCasm) {
  ...
  // Hash bytecode.
  const bytecode = stark.poseidonHashMany(casm.bytecode.map((it: string) => BigInt(it)));

  return toHex(
      stark.poseidonHashMany([
...
function hashAbi(sierra: CompiledSierra) {
  const indentString = formatSpaces(stringify(sierra.abi, undefined));
...
export function computeSierraContractClassHash(sierra: CompiledSierra) {
  ...
  // Hash Sierra program.
  const sierraProgram = stark.poseidonHashMany(sierra.sierra_program.map((it: string) => BigInt(it)));
