...
export function serializeArg(argVal: any, argType: TypeTag, serializer: Serializer) {
  if (argType instanceof TypeTagBool) {
    assertType(argVal, ["boolean", "string"]);
    if (typeof argVal === "string") {
      serializer.serializeBool(argVal.toLowerCase() === 'true');
    } else {
  ...
  if (argType instanceof TypeTagU8) {
    assertType(argVal, ["number", "string"]);
    if (typeof argVal === "string") {
      serializer.serializeU8(Number.parseInt(argVal));
    } else {
  ...
  if (argType instanceof TypeTagU64) {
    assertType(argVal, ["number", "bigint", "string"]);
    if (typeof argVal === "string") {
      serializer.serializeU64(BigInt(argVal));
    } else {
  ...
  if (argType instanceof TypeTagU128) {
    assertType(argVal, ["number", "bigint", "string"]);
    if (typeof argVal === "string") {
      serializer.serializeU128(BigInt(argVal));
    } else {
    ...
    if (argType.value instanceof TypeTagU8) {
      ...
      if (typeof argVal === "string") {
        if(argVal.startsWith("0x")) {
          serializer.serializeBytes(HexString.ensure(argVal).toUint8Array());
          return;
        }
