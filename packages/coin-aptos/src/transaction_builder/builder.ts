...
import { argToTransactionArgument, serializeArg, TypeTagParser } from './builder_utils';
...
export class TransactionBuilder<F extends SigningFn> {
  ...
  static getSigningMessage(rawTxn: AnyRawTransaction): SigningMessage {
    ...
    if (rawTxn instanceof RawTransaction) {
      hash = base.sha3_256(Buffer.from(RAW_TRANSACTION_SALT))
    } else if (rawTxn instanceof MultiAgentRawTransaction) {
      hash = base.sha3_256(Buffer.from(RAW_TRANSACTION_WITH_DATA_SALT))
    } else {
      throw new Error("Unknown transaction type.");
    }
    return Buffer.from([...hash, ...bcsToBytes(rawTxn)]);
  }
}
